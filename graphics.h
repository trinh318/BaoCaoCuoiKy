...
void deldrawText(int x, int y, const char* mess)
{
	RECT rect;
	LPCWSTR message = convertCharArrayToLPCWSTR(mess);
	HWND console_handle = GetConsoleWindow();
	...
	HPEN pen = CreatePen(PS_SOLID, 5, RGB(192, 192, 192));
	SelectObject(device_context, pen);
	SetTextColor(device_context, RGB(192, 192, 192));
	SetBkMode(device_context, TRANSPARENT);
	rect.left = x;
	rect.top = y;
	DrawText(device_context, message, -1, &rect, DT_SINGLELINE | DT_NOCLIP);
...
void setcolor(int color)
{
	...
	case 14: PEN = getPen(PS_SOLID, 2, RGB(255, 255, 255)); break;
	case 15: PEN = getPen(PS_SOLID, 2, RGB(144, 238, 144)); break;
	}
}
